...
function sass() {
    ...
    if (!PROD) {
        ...
        /* task = task.pipe($.cssnano({safe: true})); */ // khong minifier css
    }

    task = task.pipe(gulp.dest(`${site.buildRoot}/assets/css`));
    if (browser)
...
function script() {
    ...
    return task.pipe(gulp.dest(`${site.buildRoot}/assets/js`));
}
